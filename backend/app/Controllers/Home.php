...
class Home extends BaseController
{
	public function getPopup()
	{
		...
		$result = $model->getPopup($key);
		$http_origin = $_SERVER['HTTP_HOST'];
		$origin = "*";
		if ($http_origin == "store.glassprotech.com.mx" || $http_origin == "glassprotech.com.mx")
		{  
		    $origin = $http_origin;
		}
	...
	public function getCategorias()
	{
		...
		$inventory = $model->admin_getCategorias();
	...
	public function sendMail()
	{
		...
		$config['SMTPHost'] = 'mail.glassprotech.com.mx';
		$config['SMTPUser'] = 'webmaster_glassprotech@glassprotech.com.mx';

		$config['SMTPPass'] = 'J9JkR,V-8^v9';
		$config['SMTPPort'] = 465;
		$email->initialize($config);
			$mensaje = $respuesta;
			$email->setFrom("webmaster_glassprotech@glassprotech.com.mx", 'Glass Protech');
	...
	public function getArt()
	{
		...
		if($filter !== ''){
			$result = $model->getFilArt($key,$filter);
		}else{
			$result = $model->getRawArt($key);
		}
        $http_origin = $_SERVER['HTTP_HOST'];
		$origin = "*";
		if ($http_origin == "store.glassprotech.com.mx" || $http_origin == "glassprotech.com.mx")
	...
	public function usr_register()
	{
		...
		{
			$result = $model->usuarios_nuevo($key, $mail, $contra, $nombre, $phone);
			if($result!='Invalid Key'){
				$result[1] = $this->encodeUserData($result[1]);
				$result = json_encode($result);
				return $this->response->setHeader('Access-Control-Allow-Origin', $origin)
	...
	public function token_generatePswChange()
	{
		$request = \Config\Services::request();
		$model = new plataforma();
		$tokens = new tokens();
		...
		$config['SMTPPass'] = 'J9JkR,V-8^v9';
		$config['SMTPPort'] = 465;
		$email->initialize($config);				$email->setFrom('webmaster_glassprotech@glassprotech.com.mx', 'Glass Protech');
				$email->setTo($mail);
	...
	public function token_getDataForPassRecovery()
	{
		$request = \Config\Services::request();
		$tokens = new tokens();
	...
	public function token_checkToken()
	{
		$request = \Config\Services::request();
		$tokens = new tokens();
	...
	public function informacion_modificar()
	{
		...
		$result = $model->informacion_modificar($info_id,$nombre,$telefono,$calle,$ne,$ni,$colonia,$cp,$municipio,$estado,$pais);
        $http_origin = $_SERVER['HTTP_HOST'];
		$origin = "*";
		if ($http_origin == "store.glassprotech.com.mx" || $http_origin == "glassprotech.com.mx")
		{  
	...
	public function compras_newOrder()
	{
		...
		if (empty($itemErrors)) 
		{
			...
			$userInformation = $model->usuarios_getDataByID($usuario);
			$insOrder =  $model->compras_insertar("testKey", $usuario, json_encode($articulos, JSON_UNESCAPED_UNICODE),  $userInformation["usuario_información"], 1, $subtotal, $iva, $total, $userInformation["usuario_factura"], "CARD", "", "Incomplete", json_encode($sending, JSON_UNESCAPED_UNICODE), json_encode($factura, JSON_UNESCAPED_UNICODE));

			\Stripe\Stripe::setApiKey('sk_test_51IQ2HgBVvcBUZwkfMKUEA5wnmfbm1G00QYNnswQrmmIW0pEDW0hlNgvcjKv9VkfvOILMTUKsAWz5wymUtSmNULMG00H3PxjIYs');

			$c_var =  $this->encodeUserData($insOrder);

			$session = \Stripe\Checkout\Session::create([
		    'payment_method_types' => ['card'],
		    'line_items' => $itemData,
		    'mode' => 'payment',
		    'success_url' => 'https://store.glassprotech.com.mx/transaccion/exitosa?c='.$c_var,
		    'cancel_url' => 'https://store.glassprotech.com.mx/transaccion/fallida',
			]);
			
			$model->compra_updateReference($insOrder,$session->payment_intent);
			...
			$sendMail = "";
			$sendMail = $this->sendReceipt($email, json_decode(json_encode($articulos)), json_decode(json_encode($sending)), json_decode(json_encode($factura)), "card", $insOrder);
	...
	public function compra_updateStatus()
	{
		$request = \Config\Services::request();
		$model = new plataforma();

		$codigo = $request->getVar('c');
		$status = $request->getVar('status');
		$id  = $this->decodeforID($codigo);

		$compra = $model->compra_getByID($id);
		$compra = $compra[0];

		$response = "";
		if ($compra->compra_estatus == "Incomplete") 
		{
			$model->compra_updateStatus($id,$status);
			$articulos = json_decode($compra->compra_articulos);
			$sending = json_decode($compra->compra_sending);
			$email = $sending->email;
			$factura = json_decode($compra->compra_fact_data);
			$sendMail = $this->sendReceipt($email, $articulos, $sending, $factura, "card",$id);

			$response = '{"status":"ok","desc":"ok"}';
		}
		else
		{
			$response = '{"status":"error","desc":"invalid"}';
		}
		return $this->response->setHeader('Access-Control-Allow-Origin', "*")
			->setHeader('Access-Control-Allow-Headers', '*')
			->setHeader('Access-Control-Allow-Methods', 'POST')
	...
	public function compras_oxxo()
	{
		...
		if (empty($itemErrors)) 
		{
		    ...
		    $stripe = new \Stripe\StripeClient
		    (
			  'sk_test_51IQ2HgBVvcBUZwkfMKUEA5wnmfbm1G00QYNnswQrmmIW0pEDW0hlNgvcjKv9VkfvOILMTUKsAWz5wymUtSmNULMG00H3PxjIYs'
			);
			...
			try 
			{
				$insOrder = $model->compras_insertar("testKey", $usuario, json_encode($articulos, JSON_UNESCAPED_UNICODE),  $userInformation["usuario_información"], 1, $subtotal, $iva, $total, $userInformation["usuario_factura"], "OXXO", $payment_intent["id"], "Incomplete", json_encode($sending, JSON_UNESCAPED_UNICODE), json_encode($factura, JSON_UNESCAPED_UNICODE));
			...
			$sendMail = $this->sendReceipt($email, $articulos, $sending, $factura,"oxxo");
	...
	function sendReceipt($mail, $content, $sending, $facture, $source, $ide)
	{
		...
		if ($source == "oxxo") 
		{
			$viewtext = view('templates/receipt-oxxo', $data);
		}
		else if($source == "card")
		{
			$viewtext = view('templates/receipt-card', $data);
		}
		...
		$email->initialize($config);		$email->setFrom('webmaster_glassprotech@glassprotech.com.mx', 'Glass Protech');
		$email->setTo($mail);
		$email->setCC('info@glassprotech.com.mx');
		...
		for ($i=0; $i < count($arrayIncompletes) ; $i++) { 
			...
			if ($response["canceled_at"] == NULL) 
			{
				if ($response["status"] == "succeeded") 
				{
					...
					$articulos = json_decode($arrayIncompletes[$i]->compra_articulos);
					$sending = json_decode($arrayIncompletes[$i]->compra_sending);
					$email = $sending->email;
					$factura = json_decode($arrayIncompletes[$i]->compra_fact_data);
					$this->updateReceipt($email, $articulos, $sending, $factura, "succeeded");
				}
			}
			else
			{
				...
				{ 
					...
					$email = $sending->email;
					$factura = json_decode($arrayIncompletes[$i]->compra_fact_data);
					$this->updateReceipt($email, $articulos, $sending, $factura, "cancelled");
				}
			}
		}	
	}

	function updateReceipt($mail, $content, $sending, $facture, $state)
	{
		...
		if ($state == "cancelled") 
		{
			$viewtext = view('templates/update-cancelled', $data);
		}
		else if($state == "succeeded")
		{
			$viewtext = view('templates/update-succeeded', $data);
		}
		...
		$email->setSubject('Estado de compra');
		$email->setMessage($mensaje);
	...
	function admin_sendMail($enID,$proveedor,$f_envio,$f_arribo,$estatus,$referencia)
	{
		...
		$compra = $model->admin_getCompraByEnvio($enID);
	...
	public function admin_newProduct()
	{
		...
		{
			$desc = $model->admin_newProduct($prodID,$descript,$ext,$precio,$cant,$img1,$img2,$img3,$marca,$cats);
			$result = '{"status":"ok", "desc":'.json_encode($desc).'}';
			//var_dump($inventory);
		}
	...
	public function admin_updateProduct()
	{
		...
		{
			$desc = $model->admin_updateProduct($prodID,$descript,$ext,$precio,$cant,$img1,$img2,$img3,$marca,$cats);
			$result = '{"status":"ok", "desc":'.json_encode($desc).'}';
			//var_dump($inventory);
		}
	...
	public function admin_deleteCategoriaById()
	{
		$request = \Config\Services::request();
		$model = new admin();
		$cookie = $request->getVar('cookie');
		...
		{
			$desc = $model->admin_deleteCategoriaById($catID);
			$result = '{"status":"ok", "desc":'.json_encode($desc).'}';
			//var_dump($inventory);
		}
	...
	public function admin_updateCategoria()
	{
		$request = \Config\Services::request();
		$model = new admin();
		$cookie = $request->getVar('cookie');
		...
		{
			$desc = $model->admin_updateCategoria($catID,$nombre,$sub);
			$result = '{"status":"ok", "desc":'.json_encode($desc).'}';
			//var_dump($inventory);
		}
	...
	public function admin_newCategoria()
	{
		$request = \Config\Services::request();
		$model = new admin();
		$cookie = $request->getVar('cookie');
		...
		{
			$desc = $model->admin_newCategoria($nombre,$sub);
			$result = '{"status":"ok", "desc":'.json_encode($desc).'}';
			//var_dump($inventory);
		}
